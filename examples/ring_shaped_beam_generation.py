...
Generating a phase profile to shape a Gaussian beam to a ring-shaped profile
...
def target_intensity(t):
    # Target function. Ring-shaped beam with beam waist = w_E

    w_E = 600*um
    n = 2
    E = np.exp(-((np.abs(t)-1400*um)/w_E)**n)
    return E**2
...
RPD.get_phase_fun()
RPD.save_design_phase_as_image('ring_hologram.png')
RPD.save_design_phase_as_file('ring_hologram.npy')
...
F.add(SLM(
      phase_mask_function = load_phase_as_function("ring_hologram.npy", 
...
#visualize the longitudinal profile of the beam
